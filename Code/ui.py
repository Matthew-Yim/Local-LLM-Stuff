...
class PrivateGptUi:
    ...
    def mount_in_app(self, app: FastAPI, path: str) -> None:
        ...
        def public(user: dict = Depends(get_user)):
            if user:
                return RedirectResponse(url='/banana')
            else:
        ...
        <head>
            ...
            <style>
                body {
                    ...
                    background: radial-gradient(circle at center, grey, #f4f4f4);
                    display: flex;
        ...
        <body>
            <h1>ISG Private GPT</h1>
            <footer style="text-align: right">Login via Azure SSO Below</footer>
        </body>
        </html>
        """
        ...
        # Mount the Gradio app within your FastAPI app
        ...
        blocks.queue()
        logger.info("Mounting the gradio UI, at path=%s", path) # error below when path=path
        app = gr.mount_gradio_app(app, blocks, path="/banana", auth_dependency=get_user)
        # @app.get("/{path:path}")
